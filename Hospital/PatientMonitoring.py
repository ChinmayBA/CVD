...
def calculateMetrics(algorithm, testY, predict):
    ...
    recall.append(r)
    fscore.append(f)
    cnx = mysql.connector.connect(host="localhost", user="root", password="", database="cvddb")
    cursor = cnx.cursor()
    insertquery="insert into ensemble(algorithmname,accuracy,f1score,precisionscore,recall) values(%s,%s,%s,%s,%s)"
    values=(algorithm,str(a),str(f),str(p),str(r))
...
font = ('times', 16, 'bold')
title = Label(main, text='CARDIO-VASCULAR DISEASE PREDICTION USING MACHINE LEARNING')
